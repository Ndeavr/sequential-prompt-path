...
BEGIN
  ...
  -- Profil propriétaire (obligatoire avant la demande).
  SELECT id INTO v_profile_id FROM public.profiles WHERE user_id = p_user_id LIMIT 1;
  IF v_profile_id IS NULL THEN
    INSERT INTO public.profiles (user_id) VALUES (p_user_id) RETURNING id INTO v_profile_id;
  END IF;
  IF v_profile_id IS NULL THEN RAISE EXCEPTION 'profile_unavailable'; END IF;
  ...
  INSERT INTO public.leads (
    ...
    v_profile_id, v_property_id, 'contractor', p_city, 'renovation', p_category,
