...
DECLARE
  ...
  v_user_email text;
  -- Scalar prospect fields: NULL-safe when no invitation token is supplied.
  v_prospect_id uuid;
  v_p_business_name text;
  v_p_category text;
  v_p_city text;
  v_p_phone_e164 text;
  v_p_email text;
  v_p_website_url text;
  v_p_street_address text;
  v_p_postal_code text;
...
BEGIN
  IF lower(coalesce(
       ...
       _context #>> '{utm,free}',
       _context->>'offer_code',
       _context #>> '{utm,offer_code}',
       ''
     )) IN ('1', 'true', 'yes', 'free', 'free_year_local_service') THEN
    RETURN public.activate_free_service_account_from_context(_user_id, _context);
  END IF;
  ...
  IF v_token IS NOT NULL THEN
    SELECT p.id, p.business_name, p.category, p.city, p.phone_e164, p.email,
      ...
      INTO v_prospect_id, v_p_business_name, v_p_category, v_p_city, v_p_phone_e164,
           v_p_email, v_p_website_url, v_p_street_address, v_p_postal_code
    ...
    IF nullif(_context->>'prospect_id', '') IS NOT NULL
       AND (_context->>'prospect_id')::uuid <> v_prospect_id THEN
      RETURN jsonb_build_object('ok', false, 'reason', 'invitation_context_mismatch');
    END IF;
  END IF;

  v_business_name := coalesce(
    nullif(btrim(coalesce(v_p_business_name, '')), ''),
    nullif(btrim(coalesce(_context->>'business_name', '')), ''),
  ...
  VALUES (
    ...
    coalesce(v_p_category, nullif(_context->>'trade', '')),
    v_p_phone_e164, coalesce(v_p_email, v_user_email),
    v_p_website_url, v_p_street_address,
    coalesce(v_p_city, nullif(_context->>'city', '')), v_p_postal_code
  )
  ON CONFLICT (user_id) DO UPDATE SET
  ...
  UPDATE public.contractor_matching_profiles
     SET contractor_id = v_contractor_id,
         ...
         city = coalesce(city, v_p_city, nullif(_context->>'city', '')),
         trade = coalesce(trade, v_p_category, nullif(_context->>'trade', '')),
  ...
  IF v_token IS NOT NULL AND v_prospect_id IS NOT NULL THEN
    ...
    SELECT o.id INTO v_offer_id
     ...
     WHERE o.contractor_id IS NULL
       ...
       AND lower(coalesce(l.city, '')) = lower(coalesce(v_p_city, ''))
       AND (
         (v_p_phone_e164 IS NOT NULL AND regexp_replace(coalesce(l.mobile_phone, l.phone, ''), '\D', '', 'g') = regexp_replace(v_p_phone_e164, '\D', '', 'g'))
         OR (v_p_email IS NOT NULL AND lower(coalesce(l.email, '')) = lower(v_p_email))
       )
...
REVOKE ALL ON FUNCTION public.activate_my_contractor_account(uuid, text, jsonb) FROM PUBLIC, anon, authenticated;
