...
export interface CatalogPlan {
  ...
  /** Free entry plan — activation only, never sent to checkout. */
  isFree: boolean;
  stripeMonthlyPriceId: string;
  stripeYearlyPriceId: string;
  /** True only when a real yearly price exists in the catalog. */
...
async function fetchPlanCatalog(): Promise<CatalogPlan[]> {
  ...
  return (data ?? [])
    // Retired plans never reach a public grid, even if a row stays active by accident.
    .filter((row: any) => row.legacy !== true)
    .map((row: any) => {
