...
import { CheckCircle2, XCircle, AlertTriangle, Rocket, Globe, Calendar, Shield, User, Camera, CreditCard, BarChart3, FileCheck, Eye } from "lucide-react";
...
const CHECKLIST: ChecklistItem[] = [
  ...
  { code: "public_profile", label: "Profil publié", icon: Globe, blocking: true, check: (c) => !!c?.company_name && !!c?.slug },
  { code: "services", label: "Services assignés", icon: FileCheck, blocking: true, check: (c) => !!c?.primary_category },
  ...
  { code: "core_identity", label: "Coordonnées minimales", icon: Shield, blocking: true, check: (c) => !!c?.contact_email || !!c?.contact_phone },
  { code: "media_minimum", label: "Photo / Logo minimum", icon: Camera, blocking: false, check: (c) => !!c?.logo_url || !!c?.cover_image_url },
...
export default function StepActivation({ state, updateState, addEvent }: Props) {
  ...
  const checklistResults = useMemo(() => {
    return CHECKLIST.map(item => ({
      ...item,
      passed: item.check(contractor, state),
    }));
  }, [contractor, state]);
  ...
  const readinessColor = readinessPercent >= 80 ? "text-green-600" : readinessPercent >= 50 ? "text-yellow-600" : "text-red-600";
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Vérifiez la checklist et activez l'entrepreneur
          </p>
        </div>
      ...
      <Card>
        <CardContent className="pt-6">
          <div className="flex items-center gap-4 mb-4">
            ...
            <Badge variant={allBlockingPassed ? "default" : "destructive"}>
              ...
              return (
                <div
                  ...
                  className={`flex items-center justify-between p-3 rounded-lg border ${
                    item.passed ? "bg-green-50/50 dark:bg-green-950/10 border-green-200 dark:border-green-900" : item.blocking ? "bg-red-50/50 dark:bg-red-950/10 border-red-200 dark:border-red-900" : "bg-yellow-50/50 dark:bg-yellow-950/10 border-yellow-200 dark:border-yellow-900"
                  }`}
                >
                  <div className="flex items-center gap-3">
                    <Icon className={`h-4 w-4 ${item.passed ? "text-green-600" : item.blocking ? "text-red-500" : "text-yellow-500"}`} />
                    <span className="text-sm">{item.label}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    {item.blocking && !item.passed && (
                      <Badge variant="destructive" className="text-[10px]">Bloquant</Badge>
                    )}
                    {item.passed ? (
