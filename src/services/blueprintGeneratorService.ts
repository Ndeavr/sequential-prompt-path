...
async function insertRelations(relations: NonNullable<UnproGraphPayload["relations"]>): Promise<number> {
  ...
  if (relations.problem_symptoms?.length) {
    ...
    if (rows.length) {
      const { error } = await supabase.from("home_problem_symptom_edges").upsert(rows as any, { onConflict: "problem_id,symptom_id", ignoreDuplicates: true });
  ...
  if (relations.problem_causes?.length) {
    ...
    if (rows.length) {
      const { error } = await supabase.from("home_problem_cause_edges").upsert(rows as any, { onConflict: "problem_id,cause_id", ignoreDuplicates: true });
      if (!error) count += rows.length;
    }
  }
