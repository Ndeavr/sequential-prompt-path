 ...
 * Mobile-first, 1 action/screen, Alex always visible.
 */
...
export default function PageAlexGuidedOnboarding() {
    ...
    try {
      ...
      if (state.categories.primary) {
        await supabase.from("contractor_services").insert({
          ...
          service_name: state.categories.primary,
          is_primary: true,
          is_active: true,
        });
      }
      ...
      toast.error("Erreur de sauvegarde des catégories");
    } finally {
      setIsProcessing(false);
    }
  ...
  const saveTerritories = useCallback(async () => {
    ...
    try {
      ...
      toast.error("Erreur de sauvegarde des territoires");
    } finally {
      setIsProcessing(false);
    }
  }, [state.contractorId, state.territories, goTo]);

  // ── Apply promo & activate plan (0$) ──
  const activateSignaturePlan = useCallback(async () => {
    ...
    try {
      ...
      if (!promoResult?.valid) {
        toast.error(promoResult?.reason || "Code promo invalide");
        setIsProcessing(false);
        return;
      }

      // Create subscription at 0$
      ...
      await supabase.from("contractors").update({ status: "active" }).eq("id", state.contractorId);
  ...
  }, [state.contractorId, state.promoCode, user?.id, update]);
  ...
  useEffect(() => {
    ...
    const runImport = async () => {
      try {
        ...
        // Update job status
        if (job) {
          await supabase.from("extraction_jobs").update({ status: "completed" }).eq("id", job.id);
        }

        // Generate basic profile description
  ...
  const publishProfile = useCallback(async () => {
    ...
    setIsProcessing(true);
    try {
      await supabase.from("contractors").update({ status: "published" }).eq("id", state.contractorId);
      goTo("published");
  ...
  const alexMessages = useMemo(() => {
    const msgs: Record<OnboardingStep, string> = {
      welcome: "Bienvenue ! Je suis Alex, votre assistant IA. Je vais vous guider pour créer votre profil professionnel en quelques minutes.",
      business_info: "Parfait ! Dites-moi un peu plus sur votre entreprise.",
      categories: "Excellent ! Maintenant, sélectionnez votre catégorie principale et vos spécialités.",
          ...
          <AnimatePresence mode="wait">
            ...
            >
              ...
              {state.step === "importing" && (
                <ImportProgressRealtime progress={state.importProgress} />
              )}
