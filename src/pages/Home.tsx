...
const Home = () => {
  ...
  return (
    <div className="light flex flex-col min-h-screen" style={{ background: "#F0F4FA", color: "hsl(222 47% 11%)" }}>
      ...
      {/* ═══════════════════════════════════════════════════════════
          HERO — Bold, vibrant, image-rich
      ═══════════════════════════════════════════════════════════ */}
      <section className="relative overflow-hidden pb-0">
        {/* Bold gradient background */}
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute inset-0" style={{ background: "linear-gradient(170deg, #EDF2FC 0%, #E3EDFA 30%, #D6E6F9 55%, #C8DFF8 100%)" }} />
          {/* Large blue glow orb top-right */}
          <div className="absolute top-[-10%] right-[-20%] w-[80vw] h-[80vw] rounded-full" style={{ background: "radial-gradient(circle, hsl(222 90% 70% / 0.18) 0%, hsl(210 80% 75% / 0.08) 40%, transparent 70%)" }} />
          {/* Accent glow bottom-left */}
          <div className="absolute bottom-[-5%] left-[-15%] w-[55vw] h-[55vw] rounded-full" style={{ background: "radial-gradient(circle, hsl(195 100% 60% / 0.12) 0%, transparent 70%)" }} />
        </div>

        {/* Dramatic organic wave flowing down */}
        <div className="absolute bottom-[-2px] left-0 right-0 pointer-events-none z-[2]">
          <svg viewBox="0 0 1440 200" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full h-auto" preserveAspectRatio="none">
            <path d="M0 120C120 60 300 160 480 120C660 80 840 150 1020 110C1200 70 1350 130 1440 100V200H0V120Z" fill="hsl(215 80% 65%)" fillOpacity="0.07" />
            <path d="M0 150C200 100 400 180 600 140C800 100 1000 170 1200 130C1350 110 1440 150 1440 140V200H0V150Z" fill="#F0F4FA" />
          </svg>
        </div>
        ...
        <div className="relative z-10 px-5 pt-6 pb-0">
          <div className="max-w-5xl mx-auto">
            <div className="relative">
              {/* Text content — overlaps with image on right on large screens */}
              <motion.div className="relative z-10 max-w-[55%] lg:max-w-[50%]" initial="hidden" animate="visible">
                {/* Headline — BOLD */}
                <motion.h1
                  variants={fadeUp} custom={0}
                  className="font-display text-[1.85rem] sm:text-[2.75rem] md:text-[3.25rem] leading-[1.05] font-bold tracking-[-0.03em]"
                ...
                {/* Subheadline */}
                <motion.p variants={fadeUp} custom={1} className="text-body leading-relaxed mt-3 max-w-[280px]" style={{ color: "hsl(220 12% 42%)" }}>
                  Comparez, évaluez et choisissez en toute confiance.
                </motion.p>
              </motion.div>

              {/* Hero image — large, overlapping, dramatic */}
              <motion.div
                initial={{ opacity: 0, scale: 0.92, x: 20 }}
                animate={{ opacity: 1, scale: 1, x: 0 }}
                transition={{ delay: 0.2, duration: 0.9, ease: [0.16, 1, 0.3, 1] }}
                className="absolute top-[-10px] right-[-40px] sm:right-[-20px] w-[55%] sm:w-[50%] lg:w-[48%] z-0"
              >
                <img
                  src={heroHouse}
                  alt="Maison moderne de luxe"
                  className="rounded-2xl w-full object-cover aspect-[4/3]"
                  style={{ boxShadow: "0 24px 48px -12px hsl(220 40% 20% / 0.2), 0 8px 16px -4px hsl(220 40% 30% / 0.1)" }}
                />
                {/* Robot mascot — large, overlapping bottom-right */}
                <motion.img
                  src={unproRobot}
                  alt="Alex — Assistant IA UNPRO"
                  className="absolute -bottom-8 -right-4 w-32 sm:w-36 lg:w-44 drop-shadow-2xl"
            ...
            {/* Search bar — elevated card, bold shadow */}
            <motion.div variants={fadeUp} custom={2} initial="hidden" animate="visible" className="relative z-10 mt-6 max-w-md">
              <div className="rounded-2xl p-4 space-y-3" style={{ background: "white", boxShadow: "0 8px 32px -6px hsl(222 60% 50% / 0.12), 0 2px 6px hsl(220 30% 20% / 0.05)", border: "1px solid hsl(220 30% 92%)" }}>
                <p className="font-display text-body font-bold" style={{ color: "hsl(222 47% 11%)" }}>Quel type de travaux ?</p>
                <div className="flex items-center gap-2">
                  <div className="flex-1 flex items-center gap-2 rounded-xl px-3 py-2.5" style={{ background: "hsl(220 30% 97%)", border: "1px solid hsl(220 20% 90%)" }}>
                    <HomeIcon className="h-4 w-4 shrink-0" style={{ color: "hsl(220 15% 60%)" }} />
                    <input
                      type="text"
                  ...
                  <button
                    ...
                    className="h-11 w-11 shrink-0 rounded-full flex items-center justify-center transition-all active:scale-95"
                    style={{ background: "linear-gradient(135deg, #3B82F6, #06B6D4)", color: "white", boxShadow: "0 6px 20px -4px hsl(222 90% 55% / 0.45)" }}
                  >
            ...
            {/* Popular tags */}
            <motion.div variants={fadeUp} custom={3} initial="hidden" animate="visible" className="relative z-10 mt-4 pb-8">
              <p className="text-caption font-semibold mb-2 uppercase tracking-wider" style={{ color: "hsl(220 12% 50%)" }}>Populaire</p>
              <div className="flex flex-wrap gap-2">
                {popularTags.map(tag => (
                  <Link
                    key={tag.label}
                    to={isAuthenticated ? "/describe-project" : "/signup"}
                    className="inline-flex items-center gap-1.5 px-4 py-2 rounded-full text-meta font-semibold transition-all hover:shadow-md active:scale-[0.97]"
