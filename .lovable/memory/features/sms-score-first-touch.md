...
- **Blocker (2026-08-25):** untouched verified pool fails the CASL provenance gate — `source_urls` holds internal lineage only (`source_table: contractor_prospects`), origin rows are `manual_import`/`facebook` with no public source URL. Never fabricate provenance; enrich sources (RBQ/REQ/registry URLs) before SMS scale-up.
